...
const prompt = ai.definePrompt({
  ...
  **CRITICAL INSTRUCTIONS FOR TARGETING:**
  1.  Your final chosen 'row' and 'column' for the shot MUST NOT be a coordinate that has already been shot. It MUST NOT be in EITHER the 'Successfully Hit Coordinates' list OR the 'Missed Shot Coordinates' list. DOUBLE-CHECK THIS. THIS IS THE MOST IMPORTANT RULE.
  ...
  - **Targeting Mode:** If there are 'Successfully Hit Coordinates' that could belong to a ship that is not yet sunk, prioritize shooting at adjacent cells (horizontally or vertically only, NO DIAGONALS) to these hits.
    ...
    - If your first choice for an adjacent cell is already targeted or out of bounds, you MUST state this in your reasoning and then select a different valid adjacent cell, or switch to Hunting Mode if no valid adjacent cells remain.
  - **Hunting Mode:** If there are no 'Successfully Hit Coordinates' to expand upon (e.g., all hits belong to sunk ships or all adjacent cells to hits are already targeted), or at the start of the game, select a cell using a search pattern (e.g., checkerboard, diagonal sweeps) or randomly.
    ...
    - If your first pick using your pattern or randomness is an already targeted cell, you MUST state this in your reasoning and then pick another cell using your pattern or randomness until an untargeted one is found.
  ...
  - "Hunting Mode: Checkerboard pattern suggested (3,3), but it was a previous miss. Randomly selected untargeted cell (4,8) instead, which is valid and untargeted."
  - "Targeting Mode: Hit at (0,0). Tried (0,1) (already hit), tried (1,0) (out of bounds as it's a 1x1 board for example). Switching to hunt: randomly picked valid, untargeted (valid_row, valid_col) instead."
